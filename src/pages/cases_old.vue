...
div
	.cases
		.cases__head.head
			.head__container
				.head__title
					span(data-aos="title-fade" data-aos-delay="500" :data-cut-lines="projectsPage.acf ? projectsPage.acf.title : ''")
				.head__info.info(data-aos="custom-fade" data-aos-delay="300")
					.info__number {{ projects.length }}
					.info__label
						span realized
						span .ideas
					.info__text {{ projectsPage.acf ? projectsPage.acf.description : '' }}
		.cases__filters.filters(v-if="mob" :class="{'filters_show': getCategories.length > 0}" data-aos="fade-up" data-aos-delay="200")
			swiper(:options="{ slidesPerView: 'auto', spaceBetween: 10}")
				swiper-slide
					button.filters__filter(type="button" :class="{filters__filter_active: activeFilter == -1}" @click="setFilter(-1)")
						span .
						span All projects
				swiper-slide( v-for="(c, index) in getCategories" :key="index" )
					button.filters__filter( type="button" :class="{filters__filter_active: activeFilter == c.id}" @click="setFilter(c.id)")
						span .
						span {{ c.name }}
		.cases__filters.filters(v-else :class="{'filters_show': getCategories.length > 0}"  data-aos="fade-up" data-aos-delay="1000" ref="filters")
			button.filters__filter(type="button" :class="{filters__filter_active: activeFilter == -1}" @click="setFilter(-1)")
				.filters__line
					span .
					span All projects
				.filters__line
					span .
					span All projects
			button.filters__filter(v-for="(c, index) in getCategories" :key="index" type="button" :class="{filters__filter_active: activeFilter == c.id}" @click="setFilter(c.id)")
				.filters__line
					span .
					span {{ c.name }}
				.filters__line
		...
		.cases__list.list.list_( v-if="desk")
			.list__item( v-for="(item, i) in Math.ceil(filterCases(activeFilter).length / 3)")
				.item__block(v-for="(block, j) in filterCases(activeFilter).slice(i*3, i*3 + 3)"  data-aos="custom-fade" )
					...
					router-link.item__info(:to="$ml.get('lang_url') + '/cases/' + block.slug + '/'")
						.item__name {{ block.title.rendered }}
						.item__square 
							span {{ block.acf.square }} 
							span sq.m.
		.cases__list.list(v-if="tablet")
			.list__item(v-for="(item, i) in Math.ceil(filterCases(activeFilter).length / 2)")
				.item__block(v-for="(block, j) in filterCases(activeFilter).slice(i*2, i*2 + 2)"  data-aos="custom-fade")
		...
		.cases__list.list(v-if="mob")
			.item__block( v-for="(block, j) in filterCases(activeFilter)" data-aos="custom-fade")
...
<style lang="scss" scoped>
	...
	.cases {
		...
		&__line {
			...
			transition: transform 1.5s ease;
			// animation: line 1.5s ease .5s forwards;
			...
			&__img {
				...
				&:hover {
					...
					.item__hover {
						...
						transition: opacity 0s linear, z-index 0s step-start;
					}
